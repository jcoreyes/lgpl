        ...
        for c in range(num_corrections):
            ...
            for i, traj_obs in enumerate(trajs['obs']):
                ...
                if c < num_corrections - 1:
                    ...
                    #returns = trajs['rewards']
                    #correction = dataset.envs[i].corrections.gen_corr(traj_obs, trajs['infos'][-1][i])
                    #correction = trajs['rewards'][i, :].sum()
                    #correction = completion_frac
                    if c == 0 or completion_frac == completion[i, c-1]:
                        correction = 0
                    elif completion_frac > completion[i, c-1]:
                        correction = 1
                    elif completion_frac < completion[i, c-1]:
                        correction = -1
