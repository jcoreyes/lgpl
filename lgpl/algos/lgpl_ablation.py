...
class LGPLAblation:
    ...
    def __init__(self, policy, optimizer, correction_horizon=3, loss_type='cross_entropy',
                 custom_loss_fn=None, logger_output='sl.csv', dagger_interval=5, use_fullinfo=False,
                 ablation=None):
        self.policy = policy
    ...
    def run_corrections(self, dataset, dataset_name, num_corrections=1,
        ...
        for c in range(num_corrections):
            ...
            for i, traj_obs in enumerate(trajs['obs']):
                ...
                if add_to_data:
                    with torch.no_grad():
                        dist = dataset.expert_pols[i].forward(np_to_var(traj_obs))
                    # filter out if obs or actions are too big
                    actions = get_numpy(dist.prob)

                    dataset.add(traj_obs, actions, corrections[i], correction_lens[i], prev_trajs[i], hl_goals[i])

                if c < num_corrections - 1:
                    if self.ablation == 'only_prev':
                        correction_lens[:] = 1
                    corrections[i, correction_lens[i], :] = dataset.envs[i].corrections.gen_corr(traj_obs, trajs['infos'][-1][i])
                    prev_trajs[i, correction_lens[i], :] = traj_obs[::dataset.traj_subsample, :].flatten()
                    if self.ablation == 'no_traj':
                        prev_trajs[:] = 0
                correction_lens[i] += 1
                if self.ablation == 'only_prev':
                    correction_lens[:] = 2
    ...
    def train(self, train_dataset, iters=100, test_dataset=None, render=False):
