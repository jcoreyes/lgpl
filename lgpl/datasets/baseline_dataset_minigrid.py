...
class BaselineDatasetMiniGrid:
    def __init__(self, env_configs, envs, expert_pols,
        ...
        for env_idx, (env, expert_traj) in enumerate(zip(self.envs, self.expert_trajs)):
            ...
            for i, (obs, action, info) in enumerate(zip(expert_traj['obs'], expert_traj['action_probs'], expert_traj['infos'])):
                self.add([obs], [action], [subgoals_en[:1]], prev_state, self.get_hl_goal(env_idx))
    ...
    def get_hl_goal(self, env_idx):
        if self.all_goals:
            return self.envs[env_idx].all_goals_en
        else:
            return self.envs[env_idx].hl_goal
    ...
    def generate_correction(self, traj, env_idx, cuda=False, add_to_data=True,
        ...
        if add_to_data:
            self.add(bad_obs, action_probs, subgoals_lst, bad_traj_last_state, self.get_hl_goal(env_idx))
...
class BaselineDatasetMiniGrid2:
    def __init__(self, env_configs, envs, expert_pols,
                 obs_dim, action_dim, extra_dim, path_len, correction_dim, hl_goal_dim,
                 buffer_size=int(5e6), batch_size=128, max_corrections=5, all_goals=False):
        """
        ...
        self.actions = np.zeros((buffer_size, action_dim), dtype=np.float32)
        self.traj_idx = np.zeros(buffer_size, dtype=np.int64)
        self.trajs = np.zeros((buffer_size, obs_dim), dtype=np.uint8)
        self.corrections = np.zeros((buffer_size, max_corrections, correction_dim), dtype=np.float32)
        self.correction_len = np.zeros((buffer_size), dtype=np.long)
        self.hl_goals = np.zeros((buffer_size, hl_goal_dim), dtype=np.int64)
        self.all_goals = all_goals
        ...
        for i, (env, expert_pol) in enumerate(zip(envs, expert_pols)):
            ...
            self.expert_trajs.append(expert_traj)
            #print(expert_traj['infos'][-1]['subgoals_left'])
            if sum([x['has_finished'] for x in expert_traj['infos']]) > 0 and len(expert_traj['infos'][0]['curr_subgoals']) == 5:
                finished.append(i)
            #print(env.all_subgoals)
    ...
    def generate_correction(self, traj, env_idx, cuda=False, add_to_data=True,
        ...
        obs = np_to_var(np.stack(bad_obs))
        with torch.no_grad():
